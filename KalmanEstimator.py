...
class KalmanEstimator:
    ...
    def _state_transition_function(state, dt):
        ...
        # Damping factors (1.0 = no damping, 0.0 = instant stop)
        # These decay the momentum smoothly during long gaps
        v_damp = 0.98  # Velocity coasts and very slowly decays
        a_damp = 0.50  # Acceleration decays rapidly to zero

        F = np.array([
            [1, 0, 0, dt, 0, 0, dt2_2, 0, 0],
            [0, 1, 0, 0, dt, 0, 0, dt2_2, 0],
            [0, 0, 1, 0, 0, dt, 0, 0, dt2_2],
            [0, 0, 0, v_damp, 0, 0, dt, 0, 0],
            [0, 0, 0, 0, v_damp, 0, 0, dt, 0],
            [0, 0, 0, 0, 0, v_damp, 0, 0, dt],
            [0, 0, 0, 0, 0, 0, a_damp, 0, 0],
            [0, 0, 0, 0, 0, 0, 0, a_damp, 0],
            [0, 0, 0, 0, 0, 0, 0, 0, a_damp]
        ])
    ...
    def _observation_function(state):
        ...
        Observes ONLY the 3D position [x, y, z] from the 9D state.
        """
        return state[:3]
    ...
    def _create_kalman_filter(self):
        ...
        (9D state).
        """
        dim_x = self.n_dims * 3  # 9D state vector
        dim_z = self.n_dims  # 3D measurement vector (x, y, z)
        ...
        for i in range(self.n_dims):  # Apply to x, y, z independently
            # Position-Position
            Q[i, i] = q11
            # Position-Velocity and Velocity-Position
            Q[i, i + self.n_dims] = q12
            Q[i + self.n_dims, i] = q12
            # Position-Acceleration and Acceleration-Position
            Q[i, i + 2 * self.n_dims] = q13
            Q[i + 2 * self.n_dims, i] = q13
            # Velocity-Velocity
            Q[i + self.n_dims, i + self.n_dims] = q22
            # Velocity-Acceleration and Acceleration-Velocity
            Q[i + self.n_dims, i + 2 * self.n_dims] = q23
            Q[i + 2 * self.n_dims, i + self.n_dims] = q23
    ...
    def estimate_frame(self, frame_data: dict[str, np.ndarray]):
        ...
        for marker_name in self.marker_groups.keys():
            ...
            # If this marker isn't tracked yet, override
            # the filter's initial [0,0,0] position with the actual observation.
            if marker_name not in self.last_known_positions and not is_occluded:
                kf.x[:self.n_dims] = observation
            ...
            if not is_occluded:
                ...
                if estimated_pos_3d is not None:
                    kf_observation = estimated_pos_3d
                else:
